 ...
 * @subpackage Link
 */
class  Tx_Masterconfig_ViewHelpers_Link_ActionViewHelper extends Tx_Fluid_ViewHelpers_Link_ActionViewHelper {
     ...
     */
    public function render($action = NULL, array $arguments = array(), $controller = NULL, $extensionName = NULL, $pluginName = NULL, $pageUid = NULL, $pageType = 0, $noCache = FALSE, $noCacheHash = FALSE, $section = '', $format = '', Tx_Yag_Domain_Model_Item $targetItem = NULL, $linkAccessRestrictedPages = FALSE, array $additionalParams = array(), $absolute = FALSE, $addQueryString = FALSE, array $argumentsToBeExcludedFromQueryString = array()) {
        if ($targetItem) {
            $additionalParams[] = array('targetItem' => $targetItem->getUid());
        }
        $this->sessionPersistenceManagerBuilder->getInstance()->addSessionRelatedArguments($arguments);

        //adjust page
        foreach($arguments as $key => $argument) {
            foreach ($argument as $partKey => $part) {
                if ($partKey == 'pagerCollection' && $part['page'] > 20) {
                    $newPage                           = (int)ceil($part['page'] / 20);
                    $arguments[$key][$partKey]['page'] = $newPage;
                } else {
                    if ($targetItem) {
                        $arguments[$key][$partKey]['page'] = 1;
                    }
                }
            }
        }
